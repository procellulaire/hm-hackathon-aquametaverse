...
contract StakingContract {
    ...
    constructor(address _acquaToken, address _acquaNFT) {
        token = IERC20(_acquaToken);
        apeNFT = IERC721(_acquaNFT);
    }
    ...
    function stake(uint256 stake_) public returns (uint256 reward) {
        require(apeNFT.balanceOf(msg.sender) >= 1, "No Acqua NFT possessed");
